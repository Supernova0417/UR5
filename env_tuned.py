...
class UR5P2PEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, seed=0, target_mode: str = None, target_pos: list | tuple | np.ndarray = None):
        ...
        # Reward configuration (TUNED for better learning)
        # Changes from original:
        # - dist_success_thresh: 0.03 → 0.10 (easier to achieve initially)
        # - time_penalty: 0.01 → 0.001 (less penalty for exploration time)
        # - success_bonus: 100.0 → 300.0 (stronger reward signal for success)
        # - progress_scale: 5.0 → 15.0 (stronger reward for getting closer)
        self.orient_axis = os.getenv("UR5_ORIENT_AXIS", "z")  # 'x','y','z','-x','-y','-z'
        self.reward_cfg = {
            "dist_success_thresh": 0.10,   # Relaxed from 0.03 to 0.10m (10cm)
            "time_penalty": 0.001,          # Reduced from 0.01 to 0.001
            "collision_penalty": 10.0,      # Keep same
            "success_bonus": 300.0,         # Increased from 100.0 to 300.0
            "orient_weight": 0.1,           # Keep same
            "orient_power": 2.0,            # Keep same
            "progress_scale": 15.0,         # Increased from 5.0 to 15.0
        }

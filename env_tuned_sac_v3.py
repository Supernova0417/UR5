...
class UR5P2PEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, seed=0, target_mode: str = None, target_pos: list | tuple | np.ndarray = None):
        ...
        # Reward configuration (TUNED v3 for SAC - simplified rewards)
        # SAC v3: Complete redesign based on SAC v2 failure analysis
        # Key insight: SAC v2 failed because rewards were too complex
        # Changes from v2:
        # - dist_success_thresh: 0.15 → 0.20 (20cm - much easier initial goal)
        # - progress_scale: 25 → 10 (simpler, less noisy gradient)
        # - success_bonus: 500 → 200 (balanced with simplified progress)
        # - time_penalty: 0.0005 → 0.0001 (almost no penalty)
        self.orient_axis = os.getenv("UR5_ORIENT_AXIS", "z")  # 'x','y','z','-x','-y','-z'
        self.reward_cfg = {
            "dist_success_thresh": 0.20,   # Extremely relaxed to 20cm for easy initial success
            "time_penalty": 0.0001,         # Near-zero penalty for maximum exploration
            "collision_penalty": 10.0,      # Keep same
            "success_bonus": 200.0,         # Reduced for balanced learning
            "orient_weight": 0.0,           # Disabled - focus on position only
            "orient_power": 2.0,            # Keep same
            "progress_scale": 10.0,         # Reduced for simpler gradient
        }

...
class UR5P2PEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, seed=0, target_mode: str = None, target_pos: list | tuple | np.ndarray = None):
        ...
        # Reward configuration (TUNED SPECIFICALLY FOR SAC)
        # SAC requires different tuning than PPO due to off-policy nature
        # Changes from PPO tuned version:
        # - dist_success_thresh: 0.10 → 0.15 (even easier initial goal for SAC)
        # - time_penalty: 0.001 → 0.0005 (more exploration time for off-policy)
        # - success_bonus: 300.0 → 500.0 (stronger signal to overcome replay buffer noise)
        # - progress_scale: 15.0 → 25.0 (much stronger gradient for SAC learning)
        self.orient_axis = os.getenv("UR5_ORIENT_AXIS", "z")  # 'x','y','z','-x','-y','-z'
        self.reward_cfg = {
            "dist_success_thresh": 0.15,   # Relaxed to 15cm for SAC (easier initial success)
            "time_penalty": 0.0005,         # Reduced from 0.001 for more exploration
            "collision_penalty": 10.0,      # Keep same
            "success_bonus": 500.0,         # Increased from 300.0 for stronger signal
            "orient_weight": 0.1,           # Keep same
            "orient_power": 2.0,            # Keep same
            "progress_scale": 25.0,         # Increased from 15.0 for stronger gradient
        }
